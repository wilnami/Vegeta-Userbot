    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            ren_reg = sudo_reg = re.compile(pattern)
        else:
            ren_ = "\\" + CMD_HANDLER
            sudo_ = "\\" + SUDO_HANDLER
    ...
    def decorator(func):
        if not disable_edited:
            bot.add_event_handler(
                func, events.MessageEdited(**args, outgoing=True, pattern=ren_reg)
            )
        bot.add_event_handler(
            func, events.NewMessage(**args, outgoing=True, pattern=ren_reg)
        )
        if allow_sudo:
            if not disable_edited:
                bot.add_event_handler(
